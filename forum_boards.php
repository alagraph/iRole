...
<table class="forum_table clearborder panel_bg" >
  ...
  foreach ($boardlist->getBoards() as $key => $value) {
      ...
      //stampo
      echo "<tr class=\"$class\">
              <td><div>$recentImg $editTitle <a href=\"forum_topics.php?b={$value->getId()}\" id=\"title_{$value->getId()}\" class=\"board_edit\" style=\"float:left;width:350px;\">{$value->getName()}</a>$del</div><div> $editDesc <span class=\"forum_lastedit {$edit_class}\" id=\"desc_{$value->getId()}\">{$value->getDescription()}</span></div></td>
