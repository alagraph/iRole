...
<? include_headers(array('style','jquery','bubble-popup')); ?>
  ...
  foreach($groups_name_array as $key => $value) {
		...
		foreach($groupList->GetList() as $k => $v) {
			...
			if( $v->loadElements(1) > 0){ //carico solo i default, e mostro la selezione solo se ci sono alternative
				...
				foreach($v->getElements() as $e){
					$foundGroups++;
					$selector.="<input type=\"$type_sel\" id=\"grpsel_{$e->getId()}\" name=\"grp_{$key}\" value=\"{$e->getId()}\" description=\"".htmlspecialchars($v->getStatute())."\" class=\"grp_selection\" /><label class=\"help\" help=\"".htmlspecialchars($v->getStatute())."\" for=\"grpsel_{$e->getId()}\">{$e->getName()}</label><br/>\n";
				}	
