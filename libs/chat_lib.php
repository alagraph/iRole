...
private function parseToDb(){
	$query="INSERT INTO room_chat (char_id,char_id_dest,room_id,instance,datetime,message,chat_tag,special,char_ip,status)
	VALUES ('{$this->char_id}','{$this->char_id_dest}','{$this->room_id}','{$this->instance}','{$this->datetime}','{$this->message}','{$this->chat_tag}','{$this->special}','{$this->char_ip}','{$this->status}')";
...
public function setStatus($status){
	...
	if (isset($this->id)){
		$query = "UPDATE `room_chat` SET `status`". = mysql_real_escape_string($status) . "\WHERE `id`='" . $this->id . "' ";
		mysql_query($query) or die(mysql_error());
	}
